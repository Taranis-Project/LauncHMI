...
// Telem variables
const long FREQUENCY = 868063E3; // Bande à utiliser !! 
const int DELAY_BETWEEN_TRANSMISSIONS =100;
int SPREADING_FACTOR = 7;
int TX_POWER = 14; // Valeur maax admise pour FuseX (ne pas dépasser)
...
const float kpaRangeTopVoltage = 3.27; // valeur maximale renvoyée par le capteur, par l'intermédiaire d'un pont diviseur de tension. 
...
void setup() {
  ...
  TX_POWER = constrain(TX_POWER, -3, 17); // Limmits of TX_POWER dB
...
void loop() {
  ...
  /////////////////////////////////////////////////////
  SPREADING_FACTOR = constrain(SPREADING_FACTOR, 6, 12);
  ...
  if (signal_seq_recu == true && loopok == false) {
    loopok = true;
    t_depart = millis();
  }
  ...
  String serializedData = String(corr_ax) + "," + String(corr_ay) + "," + String(corr_az) + ","
                        ...
                        + String(speed) + "," + String(corr_pressure);

  // Send data via LoRa
  if(digitalRead(SW_TELEM) == LOW){
    LoRa.beginPacket();
  ...
  if(millis() - t_depart >= 1800000 && loopok == true){
    digitalWrite(LED, LOW);
    SDdata = SD.open("DATA.txt", O_WRITE | O_APPEND);
    SDdata.println("STOP");
    SDdata.close();
...
void calculateAndPrintValues() { // Fonction pour calculer et afficher les valeurs
  total -= readings[currentIndex]; // Soustraction de l'ancienne valeur à la somme totale
  readings[currentIndex] = sensorValue; // Mise à jour de la valeur actuelle dans le tableau
