...
unsigned long valeur1 = 1;
unsigned long valeur2 = 1000;
unsigned long valeur3 = 1000000;
...
void loop() {
  ...
  tempBytes = unsiglong_to_4bytes(valeur1);
  memcpy(bytes1, tempBytes, 4);

  tempBytes = unsiglong_to_4bytes(valeur2);
  memcpy(bytes2, tempBytes, 4);

  tempBytes = unsiglong_to_4bytes(valeur3);
  memcpy(bytes3, tempBytes, 4);

  Serial.print(bytes1+bytes2+bytes3+"\n");
}

unsigned char* unsiglong_to_4bytes(int valeur1) {
  unsigned char bytes[4];

  bytes[0] = (valeur1 >> 24) & 0xFF;
  bytes[1] = (valeur1 >> 16) & 0xFF;
  bytes[2] = (valeur1 >> 8) & 0xFF;
  bytes[3] = valeur1 & 0xFF;
  return bytes;
} */